...
inline std::ostream& operator<<(std::ostream& s, const libremidi::message& message)
{
  ...
  for (int i = 0U; i < nBytes; i++)
    s << "Byte " << i << " = " << static_cast<int>(message[i]) << ", ";
  if (nBytes > 0)
...
inline bool chooseMidiPort(libremidi::midi_in& libremidi)
{
  ...
  const auto ports = libremidi::observer{{}, observer_configuration_for(libremidi.get_current_api())}
                   .get_input_ports();
  unsigned int i = 0;
  const unsigned int nPorts = ports.size();
  if (nPorts == 0)
  {
  ...
  else
  {
    for (i = 0; i < nPorts; i++)
    {
      std::string portName = ports[i].display_name;
